...
train_and_evaluate <- function(k) {
	...
	# split the data into training data (first 25 samples) and validation data (last 10 samples)
	# we only take the first k features (plus the bias)
	train_data <- list(X = data$X[1:25, 1:(k + 1)], y = data$y[1:25,])
	validate_data <- list(X = data$X[26:35, 1:(k + 1)], y = data$y[26:35,])
